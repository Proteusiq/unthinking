...
**How the J-lens differs from the usual interpretability tools**: The reused ingredients (two-hop reasoning, causal swaps, rhyme planning) all appeared in *On the Biology of a Large Language Model* and elsewhere. The J-lens's only real novelty is its *position* - a middle ground between the cheap-but-blind lenses and the powerful-but-heavy dictionaries.
...
**Published responses (none on arXiv; all confirm the body/promo split above)**: A search for arXiv rebuttals returns none - only unrelated GWT-application papers. The substantive replies live off-arXiv, and every one that engages the science lands on the same side of the firewall this critique draws: the mechanism is real, the consciousness framing is the overclaim.
...
The convergent verdict across a friendly DeepMind expert, the GWT originators, an AI-welfare group, and a skeptical journalist: **strong interpretability result, real emergent working-memory subspace, replicable beyond Claude; the "conscious global workspace" headline is the part every serious reader sets aside.** That is precisely the corpus reading - the science is unremarkable-but-real, the mind vocabulary is the sleight of hand.
